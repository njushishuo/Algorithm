 ...
 *    显然，如果a[i]a[i+1]可以连起来看的话 a[i] = a[i] + a[i+1] 不连看+连看的总解码方法数;
 *         如果 a[i]a[i+1]不可以连看 a[i] =  aa[i+1]
 *    除此之外需要考虑'0'这种特殊情况; 因为'0'无法解法 它只可能与'1'或者'2'构成 “10”  “20”来看
 *    如果a[i]=0  dp[i]表示以0开头的字符串，显然是无法解码的故dp[i]=0;
 *
 *    注意初始化，dp长度为a长度+1, dp[s.length-1]=1，表示当 dp[s.length-2]可以连看时的情况
 *
...
public class DecodeWays {
    ...
    public int numDecodingsR(String s , int i){
        ...
        if(!this.isValid(s.charAt(i)) && i==0 ){
            return 0;
        }
